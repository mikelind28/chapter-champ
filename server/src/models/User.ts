...
interface SavedBook extends BookDocument {
  status: 'Want to Read' | 'Currently Reading' | 'Finished' | 'Favorite';
}
    ...
    savedBooks: [
      {
        ...
        status: {
          type: String,
          enum: ['Want to Read', 'Currently Reading', 'Finished', 'Favorite'],
...
userSchema.virtual('finishedReadingCount').get(function () {
  return this.savedBooks.filter((book: SavedBook) => book.status === 'Finished').length;
});
